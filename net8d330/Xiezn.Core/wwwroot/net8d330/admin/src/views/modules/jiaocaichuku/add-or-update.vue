	...
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230329/eff4ea96c952463cb46b9c79d043ce56.gif) fixed,#f6f6f6"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="教材分类" prop="jiaocaifenlei">
					<el-input v-model="ruleForm.jiaocaifenlei" placeholder="教材分类" clearable  :readonly="ro.jiaocaifenlei"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="教材分类" prop="jiaocaifenlei">
					<el-input v-model="ruleForm.jiaocaifenlei" placeholder="教材分类" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="出库数量" prop="shuliang">
					<el-input v-model="ruleForm.shuliang" placeholder="出库数量" clearable  :readonly="ro.shuliang"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="出库数量" prop="shuliang">
					<el-input v-model="ruleForm.shuliang" placeholder="出库数量" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="出库详情" prop="chukuxiangqing">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
					  placeholder="出库详情"
					  v-model="ruleForm.chukuxiangqing" >
					</el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.chukuxiangqing" label="出库详情" prop="chukuxiangqing">
					<span :style='{"fontSize":"16px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.chukuxiangqing}}</span>
				</el-form-item>
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='chukuxiangqing'){
							this.ruleForm.chukuxiangqing = obj[o];
							this.ro.chukuxiangqing = true;
							continue;
						}
						if(o=='yonghuzhanghao'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiaocaichuku/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiaocaichukuCrossAddOrUpdateFlag = false;
						 this.parent.search();
