...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
				...
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"border":"0px solid #f4e5e2","padding":"24rpx","margin":"0 0 40rpx","borderRadius":"24rpx","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
				  ...
				  <view class="listmr" :style='{"width":"calc(100% - 0px)","padding":"0","margin":"0","height":"auto"}'>
					...
					<view :style='{"width":"100%","padding":"16rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto","order":"5"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('zhuanye','修改')) || (!userid && isAuthFront('zhuanye','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('zhuanye','删除')) || (!userid && isAuthFront('zhuanye','删除'))" @click.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"2rpx solid #fff","boxShadow":"0 4rpx 8rpx rgba(0,0,0,.3)","color":"#fff","bottom":"120rpx","right":"128rpx","outline":"none","borderRadius":"100%","background":"#11999E","width":"88rpx","lineHeight":"88rpx","fontSize":"28rpx","position":"fixed","height":"88rpx","zIndex":"9999"}' v-if="userid && isAuth('zhuanye','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"2rpx solid #fff","boxShadow":"0 4rpx 8rpx rgba(0,0,0,.3)","color":"#fff","bottom":"120rpx","right":"128rpx","outline":"none","borderRadius":"100%","background":"#11999E","width":"88rpx","lineHeight":"88rpx","fontSize":"28rpx","position":"fixed","height":"88rpx","zIndex":"9999"}' v-if="!userid && isAuthFront('zhuanye','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`zhuanye`, params);
                } else {
                    res = await this.$api.list(`zhuanye`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('zhuanye', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`zhuanye`, searchForm);
                } else {
                    res = await this.$api.list(`zhuanye`, searchForm);
                }
				// 如果是第一页数据置空
