	...
	<view :style='{"minHeight":"100vh","padding":"0px 0 80rpx","borderColor":"#21d5ae","background":"#fff","borderWidth":"0px 0 0","width":"100%","position":"relative","borderStyle":"dashed","height":"auto"}'>
		<form :style='{"width":"100%","padding":"60rpx 40rpx","background":"none","height":"auto"}' class="app-update-pv">
			<view :style='{"padding":"0 20rpx 0px","margin":"0 0 0px","borderColor":"#d9eff0","borderRadius":"0px","flexWrap":"wrap","borderWidth":"0 0 2rpx","background":"#fff","display":"flex","width":"100%","lineHeight":"100rpx","borderStyle":"dashed","height":"auto"}' class="">
				<view :style='{"width":"auto","padding":"0 20rpx 0 0","fontSize":"28rpx","color":"#11999e","textAlign":"right","fontWeight":"600"}' class="title">年级</view>
				<input :style='{"border":"0","padding":"0px 20rpx","margin":"0px","color":"#666","borderRadius":"0px","flex":"1","background":"none","fontSize":"28rpx"}' :disabled="ro.nianji" v-model="ruleForm.nianji" placeholder="年级"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`nianji`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`nianji`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`nianji`, this.ruleForm);
						}else{
							await this.$api.add(`nianji`, this.ruleForm);
						}
